...
update_file() {
    ...
    if [ -z "$current" ]; then
        current=$(grep -oE "${token}[^0-9]*[0-9]+\.[0-9]+\.[0-9]+" "$file")
    fi

    current_ver=$(echo "$current" | grep -oE "[0-9]+\.[0-9]+\.[0-9]+" | head -n1)
...
exit_code=0
update_file "Dockerfile" "QDRANT_VERSION" || exit_code=1
update_file ".github/workflows/deploy.yml" "IMAGE_TAG" || exit_code=1
update_file "README.md" "QDRANT_VERSION" || exit_code=1
update_file "README-DOCKER.md" "QDRANT_VERSION" || exit_code=1
